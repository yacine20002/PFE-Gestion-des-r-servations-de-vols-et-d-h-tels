...
exports.deleteFlight = exports.updateFlight = exports.createFlight = exports.getFlightById = exports.getFlights = void 0;
const prisma_1 = require("../lib/prisma");
const getId = (param) => Array.isArray(param) ? param[0] : param;
const getFlights = async (req, res) => {
    try {
        const { origin, destination, departureDate, passengers = '1', minPrice, maxPrice, airline, status, page = '1', limit = '20' } = req.query;
        const where = {};
        if (origin)
...
const createFlight = async (req, res) => {
    try {
        const { flightNumber, origin, destination, departureTime, arrivalTime, price, totalSeats, airlineId } = req.body;
        // Check if manager owns the airline
        if (req.user?.role === 'FLIGHT_MANAGER') {
            const airline = await prisma_1.prisma.airlineCompany.findFirst({
                where: { id: airlineId, managers: { some: { id: req.user.id } } },
            });
            if (!airline) {
                res.status(403).json({ error: 'Not authorized for this airline' });
                return;
            }
        }
...
const updateFlight = async (req, res) => {
    try {
        const id = getId(req.params.id);
        const flight = await prisma_1.prisma.flight.findUnique({ where: { id } });
        if (!flight) {
            res.status(404).json({ error: 'Flight not found' });
            return;
        }
        ...
        const updated = await prisma_1.prisma.flight.update({
            where: { id },
            data: {
                flightNumber, origin, destination,
