...
export const processPayment = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!['flight', 'hotel'].includes(reservationType)) {
      res.status(400).json({ error: 'Invalid reservation type' }); return;
    }
    ...
    if (!paymentSuccess) {
      res.status(402).json({ error: 'Payment declined. Please try again.' }); return;
    }

    let totalAmount: number;
    let userEmail = req.user!.email;
    let paymentData: any = { amount: 0, method: paymentMethod || 'CARD', status: 'PAID', transactionId, cardLast4 };
    ...
    if (reservationType === 'flight') {
      ...
      if (reservation.paymentStatus === 'PAID') { res.status(400).json({ error: 'Already paid' }); return; }
      ...
      await createNotification(req.user!.id, 'Payment Successful', `Payment of $${totalAmount} confirmed for flight ${reservation.flight.flightNumber}.`, 'SUCCESS');
      await sendEmail({
        to: userEmail,
      ...
      await createNotification(req.user!.id, 'Payment Successful', `Payment of $${totalAmount} confirmed for ${reservation.hotel.name}.`, 'SUCCESS');
      await sendEmail({
        to: userEmail,
